...
public class EJBmoxPCMRepositoryModelCreator {
	...
	private void createRequiredRoles(final BasicComponent basicComponent, final Class ejbClass) {
		final List<Field> ejbFields = ejbClass.getMembers().stream().filter(member -> member instanceof Field)
				.map(member -> (Field) member).filter(field ->  isEJBRelevantField(field))
	...
	/**
	 * A fied is EJB relevant if it has the @EJB annotation.If extensions are enabled EJBmox also considers fields that have an corresponding architectural interface as EJB interfaces.
	...
	private boolean isEJBRelevantField(Field field) {
		...
		if(IS_EXTENSION_FOR_MEDIA_STORE_ENABLED){
			...
			OperationInterface opIf = this.sourceCodeDecoratorHelper.findPCMOperationInterfaceForJaMoPPType(accessedType);
			return null != opIf;
		}
		return false;
	}
